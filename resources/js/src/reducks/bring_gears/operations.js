...
export const getBring = (user_id) => {
  return async (dispatch, getState) => {
    const url = `/bring_gears/${user_id}`;

    await axios.get(url)
      .then((res) => {
        console.log(res)
        dispatch(getBringsAction({
      ...
      .catch((err) => {"getBringGear : ", console.log(err)});
  }
}
...
export const createBrings = (id) => {
  return async (dispatch, getState) => {
    console.log("createBringGear-----")
    const state = getState();
    const user_id = state.users.user_id;
    
    const url = `/bring_gears/${id}`;
